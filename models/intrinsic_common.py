...
def conv_f(x, stride, kernel_size, layer, subspace, pad='reflection'):
    ...
    weight = torch_sparse.spmm(i, v, layer['w_num'], subspace).view(layer['w_shape'])
    i, v = layer['bias']._indices(), layer['bias']._values()
    bias = torch_sparse.spmm(i, v, layer['b_num'], subspace).view(layer['b_shape'])
